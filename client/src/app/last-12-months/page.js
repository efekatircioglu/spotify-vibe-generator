...
export default function Last12MonthsPage() {
  ...
  useEffect(() => {
    ...
    const savedPlaylistUrl = localStorage.getItem('last12months_playlist_url');
    if (savedPlaylistUrl) {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          ...
          if (result.playlistUrl) {
            ...
            localStorage.setItem('last12months_playlist_url', result.playlistUrl);
          }
        }, 2000);
      } else {
        ...
        <div className={styles.metricsModalOverlay} onClick={() => setShowPlaylistModal(false)}>
          <div className={styles.metricsModal} onClick={e => e.stopPropagation()} style={{ minWidth: 400, maxWidth: 500 }}>
            ...
            <p style={{ textAlign: 'center', marginBottom: 16, color: '#888' }}>
              Creating playlist from: <span style={{ color: '#1db954' }}>Last 12 Months</span>
            </p>
            ...
            <div style={{ marginBottom: 16 }}>
              <p style={{ color: '#888', fontSize: '14px' }}>
                This will create a playlist with {data?.tracks?.length || 0} tracks from your last 12 months.
              </p>
            </div>
